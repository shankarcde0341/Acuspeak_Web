...
export default function InteractiveLesson({
  ...
  // Main playback trigger (handles Audio MP3 or SpeechSynthesis)
  const speakLine = useCallback(
    (index: number) => {
      ...
      if (targetLine.audio_url) {
        try {
          ...
          audio.onerror = () => {
            // Audio file failed to load -> Fallback to SpeechSynthesis
            speakSpeechSynth(index, targetLine);
          };

          audio.play().catch(() => {
            // Autoplay restricted or missing file -> Fallback to SpeechSynthesis
            speakSpeechSynth(index, targetLine);
          });
        } catch (e) {
          speakSpeechSynth(index, targetLine);
    ...
    [handleCheckCompletion, markLineCompleted, playbackSpeed, script, speakSpeechSynth]
  );
  ...
  return (
    <main className={styles.lessonContainer}>
      ...
      <header className={styles.headerCard}>
        ...
        <div className={styles.titleBlock}>
          ...
          <p className={styles.subtitle}>
            Practice dialogue listening and speaking. Click any line to listen.
          </p>
        </div>
        ...
        {script.map((line, idx) => {
          ...
          return (
            <li
              ...
              className={`${styles.chatItem} ${isLeft ? styles.chatItemAlex : styles.chatItemPriya
                }`}
            ...
            >
              ...
              <div
                className={`${styles.avatarCircle} ${isLeft ? styles.avatarAlex : styles.avatarPriya
                  }`}
                aria-hidden="true"
              >
              ...
              {/* Chat Speech Bubble */}
              <article
                className={`${styles.chatBubbleCard} ${isActive ? styles.activeBubbleCard : ''
                  }`}
              >
                <div className={styles.bubbleHeader}>
                  <span
                    className={`${styles.speakerLabel} ${isLeft ? styles.speakerAlex : styles.speakerPriya
                      }`}
                  >
                    {line.speaker}
                  </span>
      ...
      {/* Dynamic Waveform & Audio Player Section */}
      <section
      ...
      >
        ...
        <div className={styles.waveformBarContainer} aria-hidden="true">
          {waveformBars.map((height, i) => (
            <div
              key={i}
              className={`${styles.waveformBar} ${isPlaying ? styles.waveformBarActive : ''
                }`}
